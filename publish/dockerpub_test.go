...
func TestNewDockerPub(t *testing.T) {
	...
	pub, err := NewDockerPub("image", []string{"v666", "latest"}, "alpine:3.15", targets, "./fixtures/example.Dockerfile")
	require.NoError(t, err)
...
func TestDockerPub_Execute(t *testing.T) {
	...
	pub, err := NewDockerPub("image", []string{"v666", "latest"}, "alpine:3.15", targets, "../tmpl.Dockerfile")
	require.NoError(t, err)
